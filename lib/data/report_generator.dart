...
class ReportGenerator {
  Future<List<Map<String, dynamic>>> getReportData(String financialYear) async {
    ...
    for (var item in inventoryBox.values) {
      ...
      for (var receipt in stockReceiptBox.values.where(
        (r) => r.itemId == item.id && r.receiptDate.isBefore(startDate),
      )) {
        openingBalanceReceipts += receipt.quantity;
      }

      int openingBalanceIssues = 0;
      for (var issuance in issuanceBox.values.where(
        (i) => i.itemId == item.id && i.issuanceDate.isBefore(startDate),
      )) {
      ...
      for (var receipt in stockReceiptBox.values.where(
        (r) =>
            r.itemId == item.id &&
            !r.receiptDate.isBefore(startDate) &&
            r.receiptDate.isBefore(endDate),
      )) {
      ...
      for (var issuance in issuanceBox.values.where(
        (i) =>
            i.itemId == item.id &&
            !i.issuanceDate.isBefore(startDate) &&
            i.issuanceDate.isBefore(endDate),
      )) {
  ...
  Future<List<StockReceipt>> getItemStockHistory(
    String itemId,
    ...
    // if (kDebugMode) {
    //   print("ggeting info for stock history ");
    //   print(fyDates);
    //   print(startDate);
    //   print(endDate);
    // }
  ...
  Future<List<IssuanceRecord>> getItemIssuanceHistory(
    String itemId,
    String financialYear,
  ) async {
    final issuanceBox = await Hive.openBox<IssuanceRecord>('issuance_records');
    ...
    // if (kDebugMode) {
    //   print("ggeting info for issue history ");
    //   print(fyDates);
    //   print(startDate);
    //   print(endDate);
    // }
